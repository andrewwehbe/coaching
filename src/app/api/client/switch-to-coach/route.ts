import { NextResponse } from 'next/server';

import { issueSession, readSession, revokeSession } from '@/lib/auth';
import { db } from '@/lib/supabase';
import { audit } from '@/lib/audit';
import { linkForClient } from '@/lib/coach-link';

// Reverse of /api/coach/switch-to-self: lets a linked client jump back into
// their coach session without re-entering the coach PIN. Only works for a
// client that has a coach_links row pointing at them — paranoia: a hostile
// client account shouldn't be able to escalate.

export async function POST(req: Request) {
  const user = await readSession();
  if (!user || user.type !== 'client') {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }

  const link = await linkForClient(user.id);
  if (!link) {
    return NextResponse.json({ error: 'Not allowed' }, { status: 403 });
  }

  const supa = db();
  const { data: coach } = await supa
    .from('coaches')
    .select('id, name')
    .eq('id', link.coachId)
    .maybeSingle();

  if (!coach) {
    return NextResponse.json({ error: 'Linked coach not found' }, { status: 404 });
  }

  await audit({
    actorType: 'client',
    actorId: user.id,
    action: 'switch_to_coach',
    targetType: 'coach',
    targetId: coach.id,
    details: { coach_name: coach.name },
  });

  await revokeSession();
  await issueSession(
    { type: 'coach', id: coach.id, name: coach.name },
    req.headers.get('user-agent'),
  );

  return NextResponse.json({ ok: true, redirect: '/coach' });
}
