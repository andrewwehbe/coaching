-- 0037: single-round-trip set logging.
-- POST /api/client/workout/[id]/set previously cost 7-8 sequential
-- PostgREST calls (session, client, workout guard, exercise verify, log
-- upsert, set upsert, best_effort read, best_effort write). This function
-- does all of it in one call and one transaction, so a mid-write failure
-- can no longer leave a 'completed' exercise_log with zero sets.
--
-- Auth is resolved from the raw session cookie token (hashed lookup per
-- 0036, raw fallback for pre-migration rows). Best-effort comparison
-- mirrors src/lib/best-effort.ts exactly: kg-normalized, 1g tie epsilon,
-- more-reps-wins on ties, automatic PR clears a manual pin. The PR delta
-- message is built by the route from the returned prev snapshot (the
-- tested pure helper prDeltaMessage stays the single source of truth).
--
-- Returns jsonb:
--   { "error": "unauthorized" | "exercise_mismatch" | "invalid_video_path" }
--   or
--   { "ok": true, "set_id": ..., "log_id": ...,
--     "pr": null | { "prev": null | { "weight":..,"unit":..,"reps":.. } } }
...
begin
  ...
  on conflict (workout_id, exercise_id) do update set status = 'completed'
